<%-- 
    Document   : stuRo
    Created on : 28 Mar, 2019, 2:48:59 PM
    Author     : Intel
--%>
<%
...
<body>
  ...
  <div class="mid">
    ...
    <table id="t01">
        <%
            ArrayList<route> sd = (ArrayList<route>)session.getAttribute("rout");  
  ...
  <tr>
    ...
    <th>Destination</th>
    <%
       session.setAttribute("rname",sd.get(0).getRcode());
       for(int i=0;i<sd.size();i++){     
    %>
  </tr>
  <tr>
    <td><%= i+1%></td>
    <td><%= sd.get(i).getRcode() %></td>
    <td><%= sd.get(i).getRname() %></td>
    <td><%= sd.get(i).getDname() %></td>
    <td><%= sd.get(i).getPpoint() %></td>
    <td><%= sd.get(i).getTime() %></td>
    <td><%= sd.get(i).getDestination() %></td>
  </tr>
  <%
      }
}
